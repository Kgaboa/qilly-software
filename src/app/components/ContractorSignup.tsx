...
export function ContractorSignup({ onSuccess, onBack }: ContractorSignupProps) {
  ...
  const [currentStep, setCurrentStep] = useState<'details' | 'tier' | 'payment' | 'submitting'>('details');
  ...
  const handleFinalSubmit = async (payment: typeof paymentDetails) => {
    ...
    try {
      ...
      // Determine success message based on tier AND payment method
      const isManualEFT = payment?.method === 'manual';
      const requiresPayment = selectedTier === 'PROFESSIONAL' || selectedTier === 'ENTERPRISE';

      let successMessage: string;
      if (!requiresPayment) {
        successMessage = `Welcome to Qilly! Your FREE tier account is active. You can login now.`;
      } else if (isManualEFT) {
        successMessage = `Application received! Your payment reference has been recorded. An admin will verify your EFT payment (usually within 24 hours). You will receive login access once verified.`;
      } else {
        // Stitch / PayFast — instant
        successMessage = `Payment verified! Your ${selectedTierName} account is active. You can login now.`;
      }

      toast.success(successMessage, { duration: 8000 });
      
      // Sign out the user — they must go through the login page
      // (FREE and instant-pay accounts will pass the login gate immediately;
      //  manual-EFT accounts will be blocked until admin sets status='approved')
      await supabase.auth.signOut();
