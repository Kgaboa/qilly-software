...
export function PartnerLogin({ onSuccess, onBack }: PartnerLoginProps) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (roleError || userData?.role !== 'partner') {
        await supabase.auth.signOut();
        toast.error('Access denied. This account is not registered as a partner.');
        return;
      }

      sessionStorage.setItem('partner_logged_in', 'true');
      sessionStorage.setItem('partner_email', authData.user.email || email);
        ...
        <CardContent className="space-y-6">
          <form onSubmit={handleLogin} className="space-y-4">
            <div className="space-y-2">
              ...
              <Input id="email" type="email" placeholder="partner@yourcompany.com" value={email}
          ...
          <div className="text-center space-y-2">
            <p className="text-sm text-gray-600">
              ...
              <button type="button" onClick={onBack}
                className="text-blue-600 hover:text-blue-700 font-semibold">Apply Now</button>
            </p>
            <p className="text-xs text-gray-500">Need help? Contact partners@qilly.co.za</p>
          </div>
        </CardContent>
      </Card>
    </div>
